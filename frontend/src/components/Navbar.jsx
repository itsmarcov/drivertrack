...
import { areSoundsEnabled, setSoundsEnabled, playClick } from '../utils/sounds';
...
export default function Navbar() {
  ...
  const notifSections = [
    ...
    ...(isAdmin ? [{ type: 'reports', label: 'تبليغات جديدة', icon: '#0EA5E9', bg: '#ECFEFF', link: '/admin/reports', count: notifData?.reports?.length }] : []),
    ...(isAdmin ? [{ type: 'warning_signatures', label: 'توقيعات إنذار', icon: '#F97316', bg: '#FFF7ED', link: '/admin/warnings', count: notifData?.warning_signatures?.length }] : []),
    ...(isAdmin ? [{ type: 'questionnaire_responses', label: 'إجابات استبيان', icon: '#10B981', bg: '#ECFDF5', link: '/admin/questionnaires', count: notifData?.questionnaire_responses?.length }] : []),
  ].filter(s => s.count > 0);
    ...
    <>
      ...
      {isAdmin && (
        <NavDropdown label="النظام">
          ...
          <Link to="/admin/questionnaires" className={`nav-dd-link ${isActive('/admin/questionnaires')}`}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
            الاستبيانات
          </Link>
        </NavDropdown>
    ...
    <>
      ...
      {isAdmin && (
        <>
          ...
          <Link to="/admin/questionnaires" className={`nav-link ${isActive('/admin/questionnaires')}`} onClick={onClick}>الاستبيانات</Link>
        </>
      )}
    </>
  );
      ...
      <nav className={`navbar${hidden ? ' navbar-hidden' : ''}`}>
        <div className="nav-inner">
          ...
          <div className="nav-actions">
              ...
              {notifOpen && notifData && (
                <div className="nav-notif-dropdown">
                  <div className="nav-notif-header">
                    <span>الإشعارات</span>
                    <span className="nav-notif-count">{notifData.totals.late + notifData.totals.justifications + notifData.totals.registrations + notifData.totals.absence_requests + notifData.totals.reports + notifData.totals.warning_signatures + notifData.totals.questionnaire_responses} إجمالي</span>
                  </div>
                          ...
                          <Link to={link} className="nav-notif-section-btn" onClick={() => setNotifOpen(false)}>
                            <div className="nav-notif-section-btn-icon" style={{ background: bg }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={icon} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                ...
                                {type === 'reports' && <><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></>}
                                {type === 'warning_signatures' && <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M9 15l2 2 4-4"/></>}
                                {type === 'questionnaire_responses' && <><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></>}
                              </svg>
                            </div>
                            <div className="nav-notif-section-btn-body">
                              ...
                              <span className="nav-notif-section-btn-meta">
                                ...
                                {type === 'reports' && 'بانتظار المعالجة'}
                                {type === 'warning_signatures' && 'آخر 7 أيام'}
                                {type === 'questionnaire_responses' && 'آخر 7 أيام'}
                              </span>
                            </div>
            ...
            <button
              onClick={() => { const next = !soundsOn; setSoundsOn(next); setSoundsEnabled(next); if (next) playClick(); }}
              className="theme-toggle"
              title={soundsOn ? 'إيقاف الأصوات' : 'تشغيل الأصوات'}
            >
              {soundsOn ? (
