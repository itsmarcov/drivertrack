...
router.get('/:id/report', authenticate, async (req, res) => {
  ...
  doc.font('Helvetica-Bold').fontSize(22).fillColor('#E53935')
  ...
  const row = (label, value) => {
    doc.font('Helvetica-Bold').fontSize(11).fillColor('#374151').text(label, leftX, doc.y, { width: labelW, continued: true });
    doc.font('Helvetica').fillColor('#1A1A1A').text(value, { width: 400 });
    doc.moveDown(0.5);
  };
  ...
  doc.moveDown(0.5);
  doc.font('Helvetica-Bold').fillColor('#E53935')
    .text(
