...
router.get('/:id/report', authenticate, async (req, res) => {
  try {
    ...
    doc.font('ArB').fontSize(22).fillColor('#E53935')
      .text('إشعار غرامة تأخير', L, y, { width: W, align: 'right', features: { ccmp: false } });
    y += 35;
    ...
    const row = (label, value) => {
      ...
      const labelW = 100;
      tw.text(label, L, y, { width: labelW, align: 'right', features: { ccmp: false } });
      doc.font('ArR').fillColor('#1A1A1A')
        .text(String(value), L + labelW + 5, y, { width: W - labelW - 5, align: 'right', features: { ccmp: false } });
      y += 20;
    };
    ...
    doc.font('ArR').fontSize(12).fillColor('#1A1A1A')
      .text(bodyText, L, y, { width: W, align: 'right', lineGap: 4, features: { ccmp: false } });
    y = doc.y + 24;
    ...
    doc.font('ArR').fontSize(8).fillColor('#9CA3AF')
      .text('تم إصدار هذا التقرير بواسطة DriverTRACK — ' + new Date().toLocaleString('ar-DZ'), L, y, { width: W, align: 'right', features: { ccmp: false } });
