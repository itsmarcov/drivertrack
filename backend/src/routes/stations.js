...
router.get('/', authenticate, async (req, res) => {
  const stations = await queryAll('SELECT * FROM stations ORDER BY name ASC');
  res.json(stations);
});
...
router.get('/map-data', authenticate, authorize('admin', 'super_admin'), async (req, res) => {
  const stationList = await queryAll('SELECT id, name, code, latitude, longitude, commune_name, coverage_communes FROM stations ORDER BY name ASC');
...
router.post('/', authenticate, authorize('admin'), async (req, res) => {
  const { name, code, latitude, longitude, commune_name, coverage_communes } = req.body;
  ...
  const result = await run(
    'INSERT INTO stations (name, code, latitude, longitude, commune_name, coverage_communes) VALUES ($1, $2, $3, $4, $5, $6)',
    [name, code, latitude || null, longitude || null, commune_name || null, coverage_communes || null]
  );
...
router.put('/:id', authenticate, authorize('admin'), async (req, res) => {
  ...
  await run(
    'UPDATE stations SET name = $1, code = $2, latitude = $3, longitude = $4, commune_name = $5, coverage_communes = $6 WHERE id = $7',
    [newName, newCode, latitude !== undefined ? latitude : station.latitude, longitude !== undefined ? longitude : station.longitude, commune_name !== undefined ? commune_name : station.commune_name, coverage_communes !== undefined ? coverage_communes : station.coverage_communes, id]
  );
